...
void* bfs_huge_thread(void* arg)
{
	...
	int local_size_q = 0;
	int vertex ;
	if(idx == MAX_THREADS - 1) iteration = q_size/MAX_THREADS + q_size % MAX_THREADS;
	else iteration = q_size/MAX_THREADS; 
	for(i = 0; i < iteration; i++){
		vertex = q[i + idx*(q_size/MAX_THREADS)];
		for(j = 0; j < vector_pos[vertex]; j++){
			nbr = vtx[vertex][j];
		//	pthread_mutex_lock(&mutex);
			if(VISITED_CHECK[nbr] == UNVISITED){
...
/*	for(i = 0; i < MAX_THREADS; i++)
	...
	while(q_size != 0){
		...
		if(tmp_size < 16){
			...
			thread_array = (pthread_t*)malloc(MAX_THREADS*sizeof(pthread_t));
			LocalSize_array = (int*) realloc(NULL, MAX_THREADS*sizeof(int));
			LocalOffset_array = (int*) realloc(NULL, MAX_THREADS*sizeof(int));
...
int main (int argc, char* argv[]) {
	...
	printf("Num of Vertex[51238159]'s link: %d\n", vector_pos[51238159]);
